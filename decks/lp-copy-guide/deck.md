...
## （応用）本物に自動で近づける：「差分 → 修正」ループ

人が毎回見比べて直すのは大変。**“採点役”と“修正役”の2エージェントにラリーさせ、ハイスコア更新のように自動で寄せる**。

1. **採点**：本物とコピーを同じ幅でスクショ→重ねて違いを数値化（**pixelmatch / odiff**で赤く可視化、または **vision AI** に「違う点を重要順に箇条書き」）
2. **修正**：違いリストをそのまま修正役エージェントへ → コピーHTMLを直す
3. **再採点 → ループ**：似ている度が上がらなくなる／目標到達まで繰り返す

> 評価と修正を**別の係に分ける**と品質が安定（チーム型エージェントと同じ考え方）。
...
- 似ている度が**目標ライン（例：95%）超え**で停止
- **改善がほぼ止まったら**停止
- **最大ループ回数**を決める（例：10回）

**PDCAは2つの自動ループで回す**

- 公開**前**：本物との**差分**でループ（見た目を寄せる）
- 公開**後**：次の **Microsoft Clarity** で実ユーザー行動を見てループ

> 完全一致を狙うほど手間は増える。狙いは「叩き台を一瞬で・近くまで自動で」。そのまま公開はしない（法務）。
